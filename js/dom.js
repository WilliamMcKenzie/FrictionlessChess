...
function trashBot(id, index) {
    ...
    for (var i = 0; i < customBots.length; i++) {
        if (customBots[i].id == id) customBots[i] = {
            name: null
        }
    }
...
function setTemplate(type, index) {
    if (type == 'bot') {
        ...
        for (let i = 0; i < customBots.length; i++) {
          if (currentPopupIndex == customBots[i].id) {
              customBots[i].addedFunction = eval(`(${botTemplates[index]})`)
          }
       }
