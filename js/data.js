...
`function bot1(possibleMoves, gameState) {
    var possibleMoves = shuffle(possibleMoves)
    var bestMove = ""
    var bestMoveEvaluation = -100000
    var dummyGame = new Chess(gameState.fen())
    var side = gameState.turn()
    ...
    for (var i = 0; i < possibleMoves.length; i++) {
        ...
        console.log(worstMoveEvaluation, possibleMoves[i])
...
`function {BOTNAME}(possibleMoves, gameState){
    ...
    bestMove = searchFun(desiredDepth, desiredDepth, dummyGame, game.turn(), '', 1000, -1000)[2]
    
    function searchFun(depth, originalDepth, algo, rootMove, currentSafest) {
      depth -= 1
      ...
      //formated in: move value, position value (number of possible moves), root move
      var safestMove = [-Infinity, -Infinity, '']
      ...
      for (var i = 0; i < moves.length; i++) {
          ...
          //this is the move we will return to the client.
          if (depth + 1 == originalDepth) {

              //dive and return best possible case
              var search = searchFun(depth, originalDepth, algo, moves[i], safestMove[0])
              var square = moves[i].length > 2 ? findTile(moves[i]) : moves[i]
              var possibleMoves = algo.moves({
                  square: square
              }).length
    
              if (search > safestMove[0] || (search == safestMove[0] && possibleMoves > safestMove[1])) {
                  safestMove = [search, possibleMoves, moves[i]]
              }
          }
          //otherwise continue
          else if (depth > 0) {
              var search = searchFun(depth, originalDepth, algo, rootMove, currentSafest)
    
              if (search < min) {
                  min = search
              }
          }
          //at last iteration, evaluate for min
          else {
              ...
              if (curEval > max && originalDepth % 2 != 0) {
                  max = curEval
              } else if (curEval < min) {
                  min = curEval
              }
